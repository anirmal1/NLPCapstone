...
class Classifier(object):
	def __init__(self):
		...
		self.rnn_outputs = tf.concat([self.rnn_outputs_articles[:, -1, :], self.rnn_outputs_headlines[:, -1, :]], 1)
		#self.rnn_outputs = tf.concat([self.rnn_outputs_articles, self.rnn_outputs_headlines], 1)
		predicted_outputs = layers.fully_connected(self.rnn_outputs, num_outputs=OUTPUT_SIZE, activation_fn=tf.nn.sigmoid)
		...
		self.error = tf.nn.softmax_cross_entropy_with_logits(labels=self.outputs, logits=predicted_outputs)
	...
	for fold in fold_stances:
		...
		for epoch in range(10):
			...
			for i in range(len(article_batches)):
				...
				if i % 10 == 0:
					print('\tEpoch error for batch ' + str(i) + ' = ' + str(epoch_error))				
